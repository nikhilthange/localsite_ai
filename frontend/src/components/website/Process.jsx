              ...
              >
                {i < steps.length - 1 && (
                  <div
                    ...
                    style={{
                      background: `linear-gradient(90deg, ${primaryColor}40, ${primaryColor}10)`,
                    }}
                  />
                )}
                <div
                  className="w-20 h-20 rounded-2xl flex items-center justify-center mx-auto mb-6 relative z-10 transition-all duration-300 group-hover:scale-110"
                  style={{ backgroundColor: primaryColor + '12', color: primaryColor }}
                >
                  <span className="text-2xl font-bold">{String(i + 1).padStart(2, '0')}</span>
                </div>
                <h3 className="text-xl font-semibold mb-3" style={{ color: colors.text }}>{step.title}</h3>
                <p className="text-sm leading-relaxed max-w-xs mx-auto" style={{ color: colors.textSecondary }}>{step.description}</p>
              </motion.div>
            );
          })}
        </div>
